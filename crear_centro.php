...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($centro->create($name, $regional_id)) {
        $success_message = "Centro creado exitosamente.";
    } else {
        $error_message = "Error al crear el centro.";
    }
}
?>
...
<body class="bg-gradient-to-r from-blue-50 to-purple-50 min-h-screen">
    ...
    <div class="container mx-auto p-6">
        <div class="bg-white rounded-xl shadow-lg p-8 max-w-md mx-auto">
            <h2 class="text-2xl font-bold mb-6 text-center">Crear Centro</h2>
            <form method="POST" onsubmit="showModal('Centro creado exitosamente.')">
                <div class="mb-4">
                    <label class="block text-gray-700">Nombre del Centro</label>
                ...
                <div class="mb-4">
                    <label class="block text-gray-700">Regional</label>
                    <select name="regional_id" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-300" required>
                        <?php
                        $regionales = $regional->getAll();
                        foreach ($regionales as $regional) {
                            echo "<option value='{$regional['id']}'>{$regional['name']}</option>";
                        }
                        ?>
                    </select>
                </div>
