...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($auth->register($username, $password, 'instructor')) {
        $user_id = $auth->getLastInsertedUserId();

        if ($instructor->create($name, $user_id)) {
            $success_message = "Instructor creado exitosamente.";
        } else {
            $error_message = "Error al crear el instructor.";
        }
    } else {
...
<body class="bg-gradient-to-r from-blue-50 to-purple-50 min-h-screen">
    ...
    <div class="container mx-auto p-6">
        <div class="bg-white rounded-xl shadow-lg p-8 max-w-md mx-auto">
            <h2 class="text-2xl font-bold mb-6 text-center">Crear Instructor</h2>
            <form method="POST" onsubmit="showModal('Instructor creado exitosamente.')">
                <div class="mb-4">
                    <label class="block text-gray-700">Nombre del Instructor</label>
